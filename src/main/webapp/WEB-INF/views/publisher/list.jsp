...
<spring:message var="pageTitle" code="title.publisher.list" />
...
<spring:message var="confirmMessage" code="label.publisher.delete.confirm" />
<div class="content">
	<h3>${pageTitle}</h3>
  		...
  		<table class="table table-striped">
			...
			<tbody>
				<c:forEach var="publisher" items="${publishersList}">
					<tr>
						<td class="hidden-xs"><c:out value="${publisher.id}" /></td>
						<td><c:out value="${publisher.title}" /></td>
						<td class="hidden-xs"><spring:eval expression="publisher.dateUpdated" /></td>
						<td class="text-center text-nowrap">
							<a href="<c:url value='update/${publisher.id}' />" title="${editLabel}"><i class="fa fa-pencil-square-o"></i></a>
							<a href="<c:url value='delete/${publisher.id}' />" title="${deleteLabel}" class="confirm-dialog" dialog="${confirmMessage}" accept="${confirmLabel}"><i class="fa fa-times"></i></a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
