...
<div class="content">
	...
	<form:form action="." modelAttribute="activity">
		...
		<c:if test="${activity.id!=null}">
			<a href="<c:url value='/admin/activity/delete/${activity.id}' />" class="btn btn-danger btn-lg confirm-dialog" role="button" dialog="${confirmMessage}" accept="${confirmLabel}">${deleteLabel}</a>
		</c:if>
	</form:form>
</div>