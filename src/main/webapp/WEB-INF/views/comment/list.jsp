  		...
  		<table class="table table-striped">
			...
			<tbody>
				<c:forEach var="comments" items="${commentList}">
					<tr>
						<td class="hidden-xs"><c:out value="${comments.id}" /></td>
						<td><c:out value="${comments.title}" /></td>
						<td class="hidden-xs"><spring:eval expression="comments.dateUpdated" /></td>
						<td class="text-center text-nowrap">
							<a href="<c:url value='update/${comments.id}' />" title="${editLabel}"><i class="fa fa-pencil-square-o"></i></a>
							<a href="<c:url value='delete/${comments.id}' />" title="${deleteLabel}" class="confirm-dialog" dialog="${confirmMessage}" accept="${confirmLabel}"><i class="fa fa-times"></i></a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
